...
namespace LUDA::Library
{
    ...
    static int c_disassemble(lua_State* L) {
        ea_t func_addr = lua_tonumber(L, 1);
        ...
        for (ea_t addr = func->start_ea; addr < func->end_ea; ) {
            ...
            lua_setfield(L, -2, "op");
            lua_pushnumber(L, insn.ea);
            lua_setfield(L, -2, "ea");
            lua_pushnumber(L, insn.size);
            ...
            for (int i = 0; i < UA_MAXOP; i++) {
                ...
                /* Value */
                lua_pushnumber(L, op.value);
                lua_setfield(L, -2, "value");

                /* Offset/displacement */
                lua_pushnumber(L, op.addr);
